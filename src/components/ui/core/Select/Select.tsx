...
export function SelectTrigger({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center justify-between rounded-sm border border-white/10 bg-zinc-950 font-mono text-zinc-400 transition-all hover:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/20 disabled:cursor-not-allowed disabled:opacity-50",
    ...
    <AnimatePresence>
      {context.open && (
        <motion.div
          ...
          className={cn(
            "absolute z-50 mt-2 min-w-[8rem] w-full overflow-hidden rounded-sm border border-white/10 bg-zinc-950/95 p-1 shadow-2xl backdrop-blur-md",
            className
          )}
        >
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isSelected ? "text-white bg-white/[0.03]" : "text-zinc-500",
        sizeMap[context.size],
        className
      )}
    >
      ...
      {isSelected && (
        <motion.div
          ...
          className="ml-2"
        >
          <Check className="h-3 w-3 text-accent shadow-glow" />
        </motion.div>
      )}
    </div>
  );
